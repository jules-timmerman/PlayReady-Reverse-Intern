...
function processMessage(evt){
    ...
    xmlRequest.open("POST", LA_URL + "?cfg=(msg:clientinfo,persist:false,sl:150)", true);
...
function main(){
    ...
    var configurations = [
        {   // These configurations are those used by the Bitmovin test stream
    ...
    // We ask for a playready Key System Access with the above configuration
    // Remark : com.microsoft.playready is deprecated and NOT compliant with latest EME spec
    async function startRequest(...keySystems){
        for (var i = 0; i < keySystems.length; i++){
            var ks = keySystems[i];
            log("Trying with " + ks);
            var res = await navigator.requestMediaKeySystemAccess(ks, configurations).then(afterKeySystem, 
                reason =>{
                    log("Failed with " + ks)
                });
            // log(res);
    ...
    // startRequest("com.microsoft.playready.recommendation.3000", "com.microsoft.playready.recommendation", "com.microsoft.playready");
    
    if(document.getElementById("hardware").checked){
        startRequest("com.microsoft.playready.recommendation.3000");
    }else if(document.getElementById("software").checked){
        startRequest("com.microsoft.playready.recommendation");
    }else{
        log("Couldn't find a button checked");
        return;
    }
